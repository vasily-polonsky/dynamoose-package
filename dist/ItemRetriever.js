...
class ItemRetriever extends InternalPropertiesClass_1.InternalPropertiesClass {
    ...
    exec(callback) {
        ...
        const prepareForReturn = async (result) => {
            ...
            const array = (await Promise.all(result.Items.map(async (item) => this.getInternalProperties(internalProperties).settings.skipConformToSchema
                ? new model.Item(item, { "type": "fromDynamo" })
                : await new model.Item(item, { "type": "fromDynamo" }).conformToSchema({ "customTypesDynamo": true, "checkExpiredItem": true, "saveUnknown": true, "modifiers": ["get"], "type": "fromDynamo", "mapAttributes": true })))).filter((a) => Boolean(a));
...
ItemRetriever.prototype.skipConformToSchema = function () {
